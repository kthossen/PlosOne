...
class ConcatenatedCNN1DModel(nn.Module):
    def __init__(self, model1, model2,model3, model4):
        ...
        self.fc = nn.Linear(in_features=len(self.models) * output_size, out_features=48)
...
for t in range(num_epochs):
    ...
    if best_loss > val[t]:
        best_loss = val[t]
        # TODO: Save model 
        torch.save(model.state_dict(),'Datong48.pt')
...
model.load_state_dict(torch.load('Datong48.pt'))
#####################
...
import csv
data =[[48,rmse_score,mae_score,mape_score,"Datong"]]
